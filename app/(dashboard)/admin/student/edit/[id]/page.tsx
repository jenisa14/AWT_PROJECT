...
export default function EditStudent({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const [formData, setFormData] = useState({ StudentName: "", Email: "", Phone: "" });

  useEffect(() => {
    fetch(`/api/student/${id}`).then(r => r.json()).then(d => {
      if (d) setFormData({ StudentName: d.StudentName || "", Email: d.Email || "", Phone: d.Phone || "" });
      setFetching(false);
    }).catch(() => setFetching(false));
  }, [id]);

  async function update() {
    if (!formData.StudentName) return alert("Name is required");
    setLoading(true);
    try {
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3">
            ...
            >
              <Save size={16} className="mr-2" /> {loading ? "Updating..." : "Update Student"}
            </button>
          </div>
        </div>
      </div>
