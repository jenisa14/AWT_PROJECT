...
export default function AddStaff() {
  ...
  const [formData, setFormData] = useState({ StaffName: "", Email: "", Phone: "", Password: "password123", Role: "FACULTY" });

  async function save() {
    if (!formData.StaffName || !formData.Email) return alert("Required fields missing");
    setLoading(true);
    try {
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3">
            ...
            >
              <Save size={16} className="mr-2" /> {loading ? "Saving..." : "Save Staff"}
            </button>
          </div>
        </div>
      </div>
