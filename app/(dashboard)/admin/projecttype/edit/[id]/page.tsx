...
export default function EditProjectType({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/projecttype/${id}`).then(r => r.json()).then(d => {
      setName(d.ProjectTypeName || "");
  ...
  async function update() {
    ...
    try {
      ...
      window.location.href = "/admin/projecttype?msg=updated";
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3">
            ...
            >
              <Save size={16} className="mr-2" /> {loading ? "Updating..." : "Update Type"}
            </button>
          </div>
        </div>
      </div>
