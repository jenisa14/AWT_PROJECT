...
export default function AddMeeting() {
  ...
  async function save() {
    if (!formData.ProjectGroupID || !formData.MeetingDateTime) return alert("Required fields missing");
    setLoading(true);
    try {
      ...
      window.location.href = "/admin/projectmeeting?msg=added";
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Add Meeting</h2>
      </div>
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3">
            ...
            >
              <Save size={16} className="mr-2" /> {loading ? "Saving..." : "Save Meeting"}
            </button>
          </div>
        </div>
      </div>
