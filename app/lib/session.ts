...
const HANDOFF_MAX_AGE = 60; // 1 minute for one-time handoff token

/** Create a short-lived JWT to pass session via URL (for GET set-session step). */
...
/** Verify handoff token and return payload (for set-session route). */
...
/** Build Set-Cookie header for session (use in Route Handlers with redirect). */
